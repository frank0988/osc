...
#include "gpio.h"
#include "stddef.h"
// Weak 版本：bootloader 沒有 scheduler，所以提供空實作
// 當 kernel 連結 sched.c 時，sched.c 的強定義會覆蓋這些
...
void uart_send(unsigned int c) {
    // 關閉中斷以避免 race condition
    disable_irq();
    
    while (fifo_count(&tx_fifo) >= UART_FIFO_SIZE) {
        // FIFO 滿了，開啟中斷讓 ISR 有機會清空 FIFO
        enable_irq();
        asm volatile("nop");
        disable_irq();
    }
...
void uart_send_polling(unsigned int c){
...
char uart_getc() {
    // 等待 FIFO 有資料 (資料由 ISR 負責塞入)
    while (1) {
        disable_irq();
        if (fifo_count(&rx_fifo) > 0) {
            char c = fifo_pop(&rx_fifo);
            enable_irq();
            return c;
        }
        enable_irq();
...
void uart_printf(const char *fmt, ...) {
    preempt_disable();  // 禁止搶佔，確保整行輸出不會被打斷
    va_list args;
    va_start(args, fmt);
...
// Polling 版本的 printf，用於 exception handler（IRQ 被禁用時）
void uart_printf_polling(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    ...
    for (const char *p = fmt; *p != '\0'; p++) {
        ...
        switch (*p) {
            case 'c':
                uart_send_polling((char)va_arg(args, int));
                break;
            case 's':
                uart_puts_polling(va_arg(args, char *));
                break;
            case 'd': {
                if (is_long) {
                    long long v = va_arg(args, long long);
                    if (v < 0) { uart_send_polling('-'); v = -v; }
                    char buf[24]; int i = 0;
                    if (v == 0) buf[i++] = '0';
                    while (v > 0) { buf[i++] = (v % 10) + '0'; v /= 10; }
                    while (i > 0) uart_send_polling(buf[--i]);
                } else {
                    int v = va_arg(args, int);
                    if (v < 0) { uart_send_polling('-'); v = -v; }
                    char buf[12]; int i = 0;
            ...
            case 'x': {
                if (is_long) {
                    uint64_t v = va_arg(args, uint64_t);
                    for (int i = 60; i >= 0; i -= 4)
                        uart_send_polling("0123456789abcdef"[(v >> i) & 0xF]);
                } else {
                    uint32_t v = va_arg(args, uint32_t);
                    for (int i = 28; i >= 0; i -= 4)
                        uart_send_polling("0123456789abcdef"[(v >> i) & 0xF]);
